...
-- Beisitzer erzeugen / Auf die Pl�tze
function EverybodySitDown()
	...
	-- Geb�ude und darinstehende Sims locken
...
function Go()
	...
	-- MsgSay("judge", "@LTRIAL_GO@THiermit erkl�re ich den Proze� %1SN gegen %2SN f�r er�ffnet.",GetID("accuser"),GetID("accused"))
	-- MsgSay("judge", "@LTRIAL_GO@TKl�ger, was habt ihr gegen %1SN vorzubringen?",GetID("accused"))
	...
	if NumEvidences==0 then
		...
		-- Der Richter kommentiert die angewendete H�rte der Rechtssprechung und �ndert den TEV entsprechend
		...
		--accused und accuser d�rfen bestechen
...
function BeamIfNPCNotInside(SimAlias, LocatorName)
	BuildingGetRoom("courtbuilding", "Judge", "judgeroom")
	--feststellen ob der sim im gerichtsgeb�ude ist
